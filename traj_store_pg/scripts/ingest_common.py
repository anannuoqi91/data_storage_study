...
SUPPORTED_LAYOUTS = ("pg_tdengine_raw", "pg_tdengine_compact")
LEGACY_LAYOUT_ALIASES = {
    "raw": "pg_tdengine_raw",
    "compact": "pg_tdengine_compact",
}
SUPPORTED_LAYOUT_CHOICES = SUPPORTED_LAYOUTS + tuple(LEGACY_LAYOUT_ALIASES.keys())

SUPPORTED_POSTGRES_PROFILES = ("default", "bench_low_wal_compressed")
LEGACY_POSTGRES_PROFILE_ALIASES = {
    "low_wal": "bench_low_wal_compressed",
    "tuned": "bench_low_wal_compressed",
}
SUPPORTED_POSTGRES_PROFILE_CHOICES = SUPPORTED_POSTGRES_PROFILES + tuple(LEGACY_POSTGRES_PROFILE_ALIASES.keys())
...
class StorageUsage:
    sink_bytes: int
    postgres_data_bytes: int
    tdengine_data_bytes: int
...
def normalize_postgres_profile(postgres_profile: str) -> str:
    return LEGACY_POSTGRES_PROFILE_ALIASES.get(postgres_profile, postgres_profile)
...
def sink_disk_usage(
    ...
    tdengine_data_dir: Path | None = None,
) -> StorageUsage:
    return StorageUsage(
        sink_bytes=path_size_bytes(sink_dir),
        postgres_data_bytes=path_size_bytes(postgres_data_dir),
        tdengine_data_bytes=path_size_bytes(tdengine_data_dir),
    )
